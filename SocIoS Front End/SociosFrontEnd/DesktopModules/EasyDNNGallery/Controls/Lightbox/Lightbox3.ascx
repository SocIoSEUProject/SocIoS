...
<script type="text/javascript">
	...
	function EndRequestHandler(sender, args) 
	{
		...
		{
			if(sender._postBackSettings.sourceElement.id.indexOf('<%=ViewState["ModID"] %>') !== -1)
			{
										...
										<div class="EDGcontent">
											...
											<asp:Panel ID="pnlNestedGalleries" runat="server" HorizontalAlign="Center">
												<asp:DataList ID="dlNestedCategory" runat="server" DataKeyField="GalleryID" OnItemCommand="dlNestedCategory_ItemCommand"
                                                    ...
                                                    EnableViewState="False" ItemStyle-VerticalAlign="Top" ItemStyle-HorizontalAlign="Center" CssClass='<%#ViewState["FixedResponsiveNestedClass"]%>'>
												...
												<asp:GridView ID="gvNestedPaging" runat="server" AllowPaging="True" AutoGenerateColumns="False"
													BorderStyle="None" BorderWidth="0px" CssClass="pagerbox" HorizontalAlign="Center"
													DataKeyNames="GalleryID" GridLines="None" OnPageIndexChanging="gvNestedPaging_PageIndexChanging"
											...
											<asp:Panel ID="pnlTGDisplay" runat="server" HorizontalAlign="Center">
												...
												<asp:DataList ID="dlTableGallery" runat="server" CellPadding="0" RepeatDirection="Horizontal"
													...
													ItemStyle-HorizontalAlign="Center" OnItemCommand="dlTableGallery_ItemCommand"
													ItemStyle-VerticalAlign="Top" EnableViewState="False">
													<AlternatingItemStyle HorizontalAlign="Center" />
													<EditItemStyle HorizontalAlign="Center" />
													<ItemTemplate>
															...
															<div class="EDGmain">
																<div class="EDGvideo">
																	<a href='<%#FixVimeo(Eval("ShortEmbedUrl"),GetLightBoxSize(DataBinder.Eval(Container.DataItem, "Info"),DataBinder.Eval(Container.DataItem, "Filename")))%>'
																		lbnestedurl='<%#Eval("GetLighboxNestedURL")%>' rel="<%#Eval("RelLink")%>" edgmid='<%=ViewState["ModuleIDNR"]%>'
														...
														<asp:PlaceHolder ID="plFlashMov" runat="server" Visible='<%# ToDisplayVideoFlashMov("flashmov",DataBinder.Eval(Container.DataItem, "Info"),DataBinder.Eval(Container.DataItem, "FileExtension"))%>'>
															<div class="EDGmain">
																<div class="EDGvideo">
																	<a href='<%#Eval("Filename") %>' edgmid='<%=ViewState["ModuleIDNR"]%>' lbnestedurl='<%#Eval("GetLighboxNestedURL")%>'
														...
														<asp:PlaceHolder ID="plHTML5Audio" runat="server" Visible='<%# ShowHTML5Player("Audio",DataBinder.Eval(Container.DataItem, "MediaType"))%>'>
															<div class="EDGmain">
																<div class="EDGaudio">
																	<a href='<%#Eval("Filename")%>' lbnestedurl='<%#Eval("GetLighboxNestedURL")%>' edgmid='<%=ViewState["ModuleIDNR"]%>'
																		edgpid='<%#Eval("PictureID")%>' title='<%#Eval("GetDescription3")%>'>
												...
												<asp:GridView ID="gvTGPagination" runat="server" AllowPaging="True" DataKeyNames="PictureID"
													...
													GridLines="None" CssClass="pagerbox" EnableViewState="False">
													<Columns>

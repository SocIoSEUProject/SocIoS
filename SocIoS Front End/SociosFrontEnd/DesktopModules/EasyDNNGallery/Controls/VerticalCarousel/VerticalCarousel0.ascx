...
<script type="text/javascript">
	<%=ViewState["jQuery"]%>(document).ready(function($) {
		$.each(<%=ViewState["jQuery"]%>.browser, function (i) {
		if ($.browser.msie && $.browser.version < "8.0") {
			$('#IC<%=ViewState["ModulNr"]%> .slide').css('visibility', 'visible');
			$('.html_carousel<%=ViewState["ModulNr"]%>').css('overflow', 'visible');
			$('.html_carousel<%=ViewState["ModulNr"]%>').css('height', 'auto');}
		});
	});
	<%=ViewState["jQuery"]%>(document).ready(function($) {
...
{
	$('#IC<%=ViewState["ModulNr"]%> .slide a[title], #<%=ViewState["ModulNr"]%>EasyDNNGallery .EDGvideo a[title], #<%=ViewState["ModulNr"]%>EasyDNNGallery .EDGaudio a[title]').each(function() // Select all elements with the "tooltip" attribute
	{
									...
									<div class="slide">
										...
										<asp:PlaceHolder ID="plFlashFlow" runat="server" Visible='<%# ToDisplayVideoFlow("Video",DataBinder.Eval(Container.DataItem, "Info"),DataBinder.Eval(Container.DataItem, "FileExtension"),"Flow")%>'>
										 <a edgpid='<%#Eval("PictureID")%>' href='<%=ModulePath.Replace("Controls/VerticalCarousel/","")%>videoFlow.html?videourl=<%#RepVidNames(Eval("MediaType"),Eval("FileName"))%>&playerskin=<%=ViewState["VGSkin"]%>&playonload=<%=ViewState["PlayOnLoad"]%>&info=<%=ViewState["Info"]%>&description=<%#GetDescription2(DataBinder.Eval(Container.DataItem, "Description"))%>&fullscreen=<%=ViewState["AllowFullscren"]%>&iframe=true<%# RepNamesFlow(Eval("MediaType"),Eval("ShortEmbedUrl"))%>'
												rel="<%#RelLink()%>" target="<%#TargetWin()%>" title='<%#GetDescription3(DataBinder.Eval(Container.DataItem, "Description"),DataBinder.Eval(Container.DataItem, "PictureID"))%>'>
												<img id="img7" runat="server" alt='<%# GetAlt(Eval("Title"),Eval("PictureID")) %>' Width="<%#GetImageWidthInt()%>" Height="<%#GetImageHeightInt()%>"
													src='<%#GetThumbPath(DataBinder.Eval(Container.DataItem, "ViewerThumb"),DataBinder.Eval(Container.DataItem, "PictureID"))%>'
													border="0" />
											</a>
											<div>

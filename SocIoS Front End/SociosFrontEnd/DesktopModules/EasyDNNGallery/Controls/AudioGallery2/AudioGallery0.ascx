									...
									<div class="EDGcontent" style="text-align: center;">
										...
										<asp:DataList ID="dlAudioGallery" runat="server" Font-Bold="False" Font-Italic="False"
											...
											<ItemTemplate>
												...
												swfobject.embedSWF('<%=ModulePath.Replace("Controls/AudioGallery2/","")%><%=ViewState["Mp3Player"]%>', '<%#Getidname() %>', '<%=ViewState["AGwidth"]%>', '33', '9.0.0', 'js/expressInstall.swf', flashvars, params, attributes);
												});
												</script>
												</asp:PlaceHolder>
												...
												<div style="clear:both; display:block; text-align:center;"><asp:HyperLink ID="hlDownloadImage" runat="server" NavigateUrl='<%#GetImageDownURL(Eval("FileName"))%>' Visible='<%#GetDownloadVisible(Eval("HiResVersion"))%>' CssClass="downloadlink"><%=ViewState["strDownloadAudio"]%></asp:HyperLink></div>
												<div class="edg_share_link_wrapper">
													<asp:Panel ID="pnlMediaUrl" runat="server" Visible="False" CssClass="edn_link_and_share_container">
														<a href="#" class="EasyDNNGallery_silver_button edg_audio_gal_permalink_toggle<%=ViewState["ModulNr"]%>"><span><img alt="" class="icon" src="<%=ModulePath.Replace("Controls/AudioGallery2","")%>images/icons/grey_link.png"><%=ViewState["LinkThis"]%></span></a>
													</asp:Panel>
													<asp:Panel ID="pnlSocialSharing" runat="server" Visible="False" CssClass="edn_link_and_share_container">
														<a href="#" class="EasyDNNGallery_silver_button edg_audio_gal_socialsharing_toggle<%=ViewState["ModulNr"]%>"><span><img alt="" class="icon" src="<%=ModulePath.Replace("Controls/AudioGallery2","")%>images/icons/grey_plus.png"><%=ViewState["ShareThis"]%></span></a>
													</asp:Panel>
										...
										<br />
										<asp:DataList ID="repAudioList" runat="server" ShowFooter="False" ShowHeader="False"
											DataKeyField="PictureID" HorizontalAlign="Center" OnItemCommand="repAudioList_ItemCommand">
											<ItemTemplate>
												<div class="EDGmain">
													<div class="EDGimage">
										...
										<asp:GridView ID="gvAudioGalPagination" runat="server" AllowPaging="True" AutoGenerateColumns="False"
											BorderStyle="None" BorderWidth="0" DataKeyNames="PictureID" DataSourceID="odsGetAudio"
											GridLines="None" HorizontalAlign="Center" OnPageIndexChanged="gvAudioGalPagination_PageIndexChanged"
											OnSelectedIndexChanged="gvAudioGalPagination_PageIndexChanged" PageSize="1" ShowFooter="True"
...
<asp:ObjectDataSource ID="odsGetAudio" TypeName="DataAcess" runat="server" SelectMethod="GetAudioFromGallery">
	<SelectParameters>

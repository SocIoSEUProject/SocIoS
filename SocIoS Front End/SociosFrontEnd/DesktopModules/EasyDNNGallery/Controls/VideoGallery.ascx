										...
										<asp:DataList ID="dlVideos" runat="server" Font-Bold="False" Font-Italic="False"
											...
											<ItemTemplate>
												...
												<asp:PlaceHolder ID="phSWfFlash" runat="server" Visible='<%# ToDisplayVideo("Video",DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "Info"),DataBinder.Eval(Container.DataItem, "FileName")) %>'>
													...
													<div class="VideoDescription">
														<asp:Label ID="lblVGEVMediaDescription3" runat="server" Text='<%# ShowDescription(DataBinder.Eval(Container.DataItem, "Description"),DataBinder.Eval(Container.DataItem, "PictureID"))%>'></asp:Label>
													</div>
													<script type="text/javascript">
															<%=ViewState["jQuery"]%>(document).ready(function() {
															var flashvars = {};
												...
												<asp:PlaceHolder ID="phEVVideoFlow" runat="server" Visible='<%# ToDisplayVideo2Flow("Video",DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "Info"),DataBinder.Eval(Container.DataItem, "FileName"),"Flow") %>'>
													...
													<div class="VideoBodyPlayer">
														<a href='<%#GetFlowFileName(DataBinder.Eval(Container.DataItem, "FileName"))%>' style='display: block;
															...
															id='player<%#Eval("PictureID")%><%=ViewState["ModulNr"]%>'></a>
														<script type="text/javascript">
														flowplayer('player<%#Eval("PictureID")%><%=ViewState["ModulNr"]%>',{scaling:'<%=ViewState["FlowScaling"]%>',allowfullscreen: <%= ViewState["AllowFullFlow"]%>,src:'<%=ModulePath.Replace("Controls/","")%>flowplayer.swf',wmode: "transparent"},{
														clip: {
												...
												<script type="text/javascript">
													<%=ViewState["jQuery"]%>(document).ready(function(){
													<%=ViewState["jQuery"]%>('#Div<%#Eval("PictureID")%>video_player').H5Video({
														events : {
															pause : function()
															{
												...
												<asp:Panel ID="pnlMediaUrl" runat="server" Visible="False" CssClass="edn_link_and_share_container">
													<a href="#" class="EasyDNNGallery_silver_button edg_video_gal_permalink_toggle<%=ViewState["ModulNr"]%>"><span><img alt="" class="icon" src="<%=ModulePath.Replace("Controls/","")%>images/icons/grey_link.png"><%=ViewState["LinkThis"]%></span></a>
												</asp:Panel>
												<asp:Panel ID="pnlSocialSharing" runat="server" Visible="False" CssClass="edn_link_and_share_container">
													<a href="#" class="EasyDNNGallery_silver_button edg_video_gal_socialsharing_toggle<%=ViewState["ModulNr"]%>"><span><img alt="" class="icon" src="<%=ModulePath.Replace("Controls/","")%>images/icons/grey_plus.png"><%=ViewState["ShareThis"]%></span></a>
												</asp:Panel>
